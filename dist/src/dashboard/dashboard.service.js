...
let DashboardService = class DashboardService {
    ...
    async getWidget1(year) {
        try {
            ...
            console.error('[DASHBOARD] Widget 1 error:', e);
            return { value: 0, dataset: new Array(12).fill(0) };
        }
    }
    async getWidget2(year) {
        try {
            const start = new Date(`${year}-01-01`);
            const end = new Date(`${year + 1}-01-01`);
            ...
            console.error('[DASHBOARD] Widget 2 error:', e);
            return { value: 0, dataset: new Array(12).fill(0) };
        }
    }
    async getWidget3(year) {
        try {
            const start = new Date(`${year}-01-01`);
            const end = new Date(`${year + 1}-01-01`);
            const data = await this.prisma.abonnement.findMany({
            ...
            console.error('[DASHBOARD] Widget 3 error:', e);
            return { value: 0, datasets: [] };
        }
    }
    async getWidget4() {
        try {
            ...
            const cityIds = stats.map(s => s.ville_id).filter(v => v !== null);
            ...
            console.error('[DASHBOARD] Widget 4 error:', e);
            return [];
        }
    }
    async getWidget5() {
        try {
            ...
            console.error('[DASHBOARD] Widget 5 error:', e);
            return [];
        }
    }
    async getWidget6() {
        try {
            ...
            console.error('[DASHBOARD] Widget 6 error:', e);
            return [];
        }
    }
    async getWidget7(year) {
        try {
            const start = new Date(`${year}-01-01`);
            const end = new Date(`${year + 1}-01-01`);
            ...
            console.error('[DASHBOARD] Widget 7 error:', e);
            return { totalVendus: 0, totalUtilises: 0, datasets: [] };
        }
    }
    async getWidget8(year) {
        try {
            ...
            console.error('[DASHBOARD] Widget 8 error:', e);
            return [];
        }
    }
    async getWidget8_1(year) {
        try {
            ...
            console.error('[DASHBOARD] Widget 8_1 error:', e);
            return [];
        }
    }
    async getWidget12(year) {
        try {
            const start = new Date(`${year}-01-01`);
            const end = new Date(`${year + 1}-01-01`);
            ...
            console.error('[DASHBOARD] Widget 12 error:', e);
            return { totalAcheteurs: 0, totalFournisseurs: 0, datasets: [] };
        }
    }
    async getWidget13() {
        try {
            ...
            const totalF = datasets[0].data.reduce((a, b) => Number(a) + Number(b), 0);
            const totalA = datasets[1].data.push(0);
            return {
                years,
            ...
            console.error('[DASHBOARD] Widget 13 error:', e);
    ...
    async getDemandeAbonnements(itemsPerPage = 5) {
        try {
            ...
            console.error('[DASHBOARD] Demandes Abonnement error:', e);
    ...
    async getDemandeJetons(itemsPerPage = 5) {
        try {
            ...
            console.error('[DASHBOARD] Demandes Jeton error:', e);
            return { 'hydra:member': [], 'hydra:totalItems': 0 };
        }
    }
    async getAcheteurWidgets(id) {
        const now = new Date();
    ...
    async getDemandesBudgets(id, year) {
        const start = new Date(`${year}-01-01`);
        const end = new Date(`${year + 1}-01-01`);
    ...
    async getDemandesCharts(id, startDate, endDate) {
        return {
    ...
    async getFournisseurDoughnut(id, year) {
        return {
    ...
    async getFournisseurTopBudget(id, year) {
        return {
            labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'],
            datasets: [{
    ...
    async getFournisseurPotentiel(id, year) {
        return { score: 78, message: "Votre profil est attractif" };
    }
    async getFournisseurDemandeDevisByProduct(id) {
        return [
    ...
    async getFournisseurPersonnelsRank(id, year) {
        return [
            { id: 1, name: 'Vendeur Principal', total: 45000 }
        ];
    }
    async getBadgeCount(badge) {
        try {
            switch (badge) {
                case 'demandes-admin':
            ...
            console.error(`[DASHBOARD] Badge ${badge} error:`, e);
            return 0;
        }
    }
};
